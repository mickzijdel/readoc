...
A fresh clone or CI checkout receives the GIT INDEX mode, not the local
...
def test_shebang_scripts_are_executable_in_git_index():
    ...
    bad = [
        ...
        if mode == "100644" and _has_shebang(REPO_ROOT / path)
    ]
    assert not bad, (
    ...
    assert modes.get("bin/readoc") == "100755"
